...
function render() {
  ...
  stats.textContent = query
    ? `검색 결과 ${matches.length.toLocaleString("ko-KR")}개 / 전체 ${all.length.toLocaleString("ko-KR")}개 항목`
    : `전체 ${all.length.toLocaleString("ko-KR")}개 항목 중 주요 항목 80개 표시`;

  results.innerHTML = matches.slice(0, 160).map(item => `
    <article class="para-card">
      <small>항목 ${item.n}</small>
      <p>${highlight(item.text, query)}</p>
    </article>
  `).join("");

  if (matches.length > 160) {
    results.insertAdjacentHTML("beforeend", `<article class="para-card"><p>검색 결과가 많아 상위 160개만 표시합니다. 검색어를 더 구체적으로 입력해 주세요.</p></article>`);
  }
}
